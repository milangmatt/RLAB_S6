...
simple_linear_regression <- function() {
  ...
  mse <- mean((data$salary - predicted_values)^2)
  print(paste("Mean Squared Error (MSE):", round(mse, 2)))
  
  mae <- mean(abs(data$salary - predicted_values))
  print(paste("Mean Absolute Error (MAE):", round(mae, 2)))
  
  # Plot the predicted point
  points(experience_years, predicted_salary, col = "black", pch = 19, cex = 2)
  text(experience_years, predicted_salary, labels = round(predicted_salary, 2), pos = 4, col = "black")
}
